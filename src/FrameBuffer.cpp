...
FrameBuffer::FrameBuffer(int ctype)
{
    ...
    {
        // create texture object to depth stencil texture attachment
        glGenTextures(1, &m_RenderId);
        glBindTexture(GL_TEXTURE_2D, m_RenderId);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, Tex_Width, Tex_Height, 0, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, NULL);
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, m_RenderId, 0);
    }
